// FSRCNN v3 + Micro-Compensator (8 features, 2 layers, OpenMP)
...
// Micro-compensator functions
void micro_compensator_forward(double *input, double *output, int H, int W);
void load_micro_compensator(const char *filename);

// Micro-compensator weights (8 features, 2 layers)
// Layer 1: Conv2d(1, 8, 3x3) = 8*1*9 = 72 weights + 8 biases
// Layer 2: Conv2d(8, 1, 3x3) = 1*8*9 = 72 weights + 1 bias
#define MC_FEAT 8
double mc_w1[MC_FEAT * 1 * 9];   // 72
double mc_b1[MC_FEAT];           // 8
double mc_w2[1 * MC_FEAT * 9];   // 72
double mc_b2[1];                 // 1
int mc_loaded = 0;
...
int main(int argc, char *argv[])
{
	...
	weights_layer8_ptr = fopen("weights_layer8_finetuned_v3.txt", "r");
	...
	biases_layer8_ptr = fopen("biasess_layer8_finetuned_v3.txt", "r");
	...
	// Load micro-compensator weights
	load_micro_compensator("micro_compensator_weights.txt");
	...
	// Buffer for compensator output
	double *comp_buf = (double *)malloc(outCols*outRows*sizeof(double));
	...
	for (int fcnt = 0; fcnt < num; fcnt++)
	{
		...
		// Micro-compensator: refine output (operates on 0~1 range)
		if (mc_loaded) {
			micro_compensator_forward(outP_tmp, comp_buf, outRows, outCols);
			memcpy(outP_tmp, comp_buf, outCols * outRows * sizeof(double));
		}
...
// ==================== MICRO-COMPENSATOR ====================
// 2-layer CNN: Conv(1→8, 3x3) → ReLU → Conv(8→1, 3x3)
// Total: 217 parameters, OpenMP parallelized

void load_micro_compensator(const char *filename) {
	FILE *fp = fopen(filename, "r");
	if (!fp) {
		printf("Warning: micro_compensator_weights.txt not found, skipping compensator\n");
		mc_loaded = 0;
		return;
	}

	// Total params: 72 + 8 + 72 + 1 = 153
	// But PyTorch exports: conv1.weight(72), conv1.bias(8), conv2.weight(72), conv2.bias(1)
	int total = 72 + MC_FEAT + 72 + 1;  // 153
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (sscanf(line, "%lf", &val) == 1) {
			if (idx < total) all[idx++] = val;
		}
	}
	fclose(fp);

	if (idx != total) {
		printf("Warning: read %d values, expected %d\n", idx, total);
	}

	// Copy to arrays
	int off = 0;
	memcpy(mc_w1, all + off, 72 * sizeof(double)); off += 72;
	memcpy(mc_b1, all + off, MC_FEAT * sizeof(double)); off += MC_FEAT;
	memcpy(mc_w2, all + off, 72 * sizeof(double)); off += 72;
	memcpy(mc_b2, all + off, 1 * sizeof(double));

	free(all);
	mc_loaded = 1;
	printf("Micro-compensator loaded: %d params (8 features, 2 layers)\n", total);
}

void micro_compensator_forward(double *input, double *output, int H, int W) {
	int HW = H * W;

	// Layer 1: Conv(1→8, 3x3, pad=1) + ReLU
	double *feat = (double*)calloc(MC_FEAT * HW, sizeof(double));

	#pragma omp parallel for
	for (int oc = 0; oc < MC_FEAT; oc++) {
		double b = mc_b1[oc];
		double *w = mc_w1 + oc * 9;  // 1 input channel × 9
				...
				for (int ki = -1; ki <= 1; ki++) {
					for (int kj = -1; kj <= 1; kj++) {
						...
						if (ii >= 0 && ii < H && jj >= 0 && jj < W) {
							sum += input[ii * W + jj] * w[(ki + 1) * 3 + (kj + 1)];
						}
					}
				}
				// ReLU
				feat[oc * HW + i * W + j] = sum > 0 ? sum : 0;
			}
		}
	}

	// Layer 2: Conv(8→1, 3x3, pad=1)
	#pragma omp parallel for
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			double sum = mc_b2[0];
			for (int ic = 0; ic < MC_FEAT; ic++) {
				double *w = mc_w2 + ic * 9;
				for (int ki = -1; ki <= 1; ki++) {
					for (int kj = -1; kj <= 1; kj++) {
						int ii = i + ki;
						int jj = j + kj;
						if (ii >= 0 && ii < H && jj >= 0 && jj < W) {
							sum += feat[ic * HW + ii * W + jj] * w[(ki + 1) * 3 + (kj + 1)];
						}
					}
				}
			}
