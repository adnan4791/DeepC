...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale)
{
	...
	// Setiap piksel HR mengumpulkan kontribusi dari semua channel sekaligus.
	#pragma omp parallel for
	for (int p = 0; p < hr_pixels; p++)
	{
		...
		// Posisi di output padded (sebelum crop)
		int i_tmp = oy + ((fsize8 + 1) / 2) + scale * border8 - 1;
		int j_tmp = ox + ((fsize8 + 1) / 2) + scale * border8 - 1;

		int cols_out_pad = cols_pad8 * scale;

		double sum = biases_layer8;

		for (int ch = 0; ch < num_channels8; ch++)
		{
			double *padded = all_padded + ch * rows_pad8 * cols_pad8;
			double *kern = weights_layer8 + ch * filtersize8;

			double ch_val = 0.0;
			for (int k_r = 0; k_r < fsize8; k_r++)
			{
				int rem_r = i_tmp - k_r;
				if (rem_r < 0 || rem_r % scale != 0) continue;
				int ii = rem_r / scale;
				if (ii >= rows_pad8) continue;

				for (int k_c = 0; k_c < fsize8; k_c++)
				{
					int rem_c = j_tmp - k_c;
					if (rem_c < 0 || rem_c % scale != 0) continue;
					int jj = rem_c / scale;
					if (jj >= cols_pad8) continue;
