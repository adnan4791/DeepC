...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale, double *layer_times);
...
int main(int argc, char *argv[])
{
	...
	printf("\n=========================================================\n");
	printf("  FSRCNN Performance Benchmark (Parallel New)\n");
	printf("=========================================================\n");
	printf("  Input:  %dx%d | Output: %dx%d | Scale: %d\n", inCols, inRows, outCols, outRows, scale);
	printf("  Frames: %d | OpenMP threads: %d\n", num, num_threads);
	printf("---------------------------------------------------------\n");

	double total_start = omp_get_wtime();
	double frame_times[num];
	double layer_times[8];  // per-layer accumulated time
	for (int lt = 0; lt < 8; lt++) layer_times[lt] = 0.0;
	double min_frame_time = DBL_MAX;
	...
	for (int fcnt = 0; fcnt < num; fcnt++)
	{
		...
		double frame_start = omp_get_wtime();
		FSRCNN(outP_tmp, inP_tmp, inRows, inCols, scale, layer_times);
		...
		double frame_end = omp_get_wtime();
		frame_times[fcnt] = frame_end - frame_start;
		if (frame_times[fcnt] < min_frame_time) min_frame_time = frame_times[fcnt];
		if (frame_times[fcnt] > max_frame_time) max_frame_time = frame_times[fcnt];

		if (fcnt < 5 || fcnt == num - 1) {
			printf("  Frame %3d/%d: %.4f sec (%.2f FPS)\n",
				fcnt + 1, num, frame_times[fcnt], 1.0 / frame_times[fcnt]);
		} else if (fcnt == 5) {
			printf("  ...\n");
		}
	}
	...
	// ============ PERFORMANCE SUMMARY ============
	double avg_frame_time = total_time / num;
	double fps = num / total_time;

	// Compute total FLOPs estimation
	long long pixels_lr = (long long)inRows * inCols;
	long long pixels_hr = (long long)outRows * outCols;

	// Layer 1: 1 channel * 56 filters * 5x5 kernel = 56 * 25 * 2 FLOPs per pixel
	long long flops_l1 = pixels_lr * 56 * 25 * 2;
	// Layer 2: 56 channels * 12 filters * 1x1 kernel
	long long flops_l2 = pixels_lr * 12 * 56 * 1 * 2;
	// Layer 3-6: 12 channels * 12 filters * 3x3 kernel (x4 layers)
	long long flops_l3to6 = pixels_lr * 12 * 12 * 9 * 2 * 4;
	// Layer 7: 12 channels * 56 filters * 1x1 kernel
	long long flops_l7 = pixels_lr * 56 * 12 * 1 * 2;
	// Layer 8: 56 channels * 1 filter * 9x9 deconv
	long long flops_l8 = pixels_hr * 56 * 81 * 2;
	// PReLU: ~4 ops per pixel per filter (add, compare, multiply, add)
	long long flops_prelu = pixels_lr * (56 + 12*5 + 56) * 4;

	long long total_flops_per_frame = flops_l1 + flops_l2 + flops_l3to6 + flops_l7 + flops_l8 + flops_prelu;
	long long total_flops = total_flops_per_frame * num;
	double gflops = (double)total_flops / 1e9;
	double gflops_per_sec = gflops / total_time;

	// Memory throughput estimation (bytes read + written)
	long long bytes_per_frame = (pixels_lr + pixels_hr) * sizeof(double) * 2; // rough estimate
	long long total_bytes = bytes_per_frame * num;
	double mb_total = (double)total_bytes / (1024.0 * 1024.0);
	double mb_per_sec = mb_total / total_time;

	printf("\n=========================================================\n");
	printf("  PERFORMANCE SUMMARY\n");
	printf("=========================================================\n");
	printf("  Total Processing Time : %.4f sec\n", total_time);
	printf("  Total Frames          : %d\n", num);
	printf("---------------------------------------------------------\n");
	printf("  Avg Frame Time        : %.4f sec\n", avg_frame_time);
	printf("  Min Frame Time        : %.4f sec\n", min_frame_time);
	printf("  Max Frame Time        : %.4f sec\n", max_frame_time);
	printf("  Frames Per Second     : %.2f FPS\n", fps);
	printf("---------------------------------------------------------\n");
	printf("  Per-Layer Time Breakdown (accumulated over %d frames):\n", num);
	double layer_total = 0;
	char *layer_names[] = {"Layer 1 (Conv 5x5, 1->56)",
	                       "Layer 2 (Conv 1x1, 56->12)",
	                       "Layer 3 (Conv 3x3, 12->12)",
	                       "Layer 4 (Conv 3x3, 12->12)",
	                       "Layer 5 (Conv 3x3, 12->12)",
	                       "Layer 6 (Conv 3x3, 12->12)",
	                       "Layer 7 (Conv 1x1, 12->56)",
	                       "Layer 8 (Deconv 9x9, 56->1)"};
	for (int lt = 0; lt < 8; lt++) {
		layer_total += layer_times[lt];
		printf("    %-30s: %7.4f sec (%5.1f%%)\n",
			layer_names[lt], layer_times[lt],
			100.0 * layer_times[lt] / total_time);
	}
	printf("    %-30s: %7.4f sec (%5.1f%%)\n",
		"Other (I/O, preprocessing)",
		total_time - layer_total,
		100.0 * (total_time - layer_total) / total_time);
	printf("---------------------------------------------------------\n");
	printf("  Computational Performance:\n");
	printf("    Total FLOPs (est.)  : %.2f GFLOP\n", gflops);
	printf("    Throughput          : %.2f GFLOPS\n", gflops_per_sec);
	printf("    Data Processed      : %.2f MB\n", mb_total);
	printf("    Memory Bandwidth    : %.2f MB/s\n", mb_per_sec);
	printf("  OpenMP Threads        : %d\n", num_threads);
	printf("  Speedup Potential     : Ideal %.1fx (with %d threads)\n",
...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale, double *layer_times)
{
	...
	// Ini mencegah Stack Overflow dari array VLA dan mengisolasi tiap thread
	t_start = omp_get_wtime();
	double *all_tmp = (double *)malloc(num_channels8 * hr_pixels * sizeof(double));
	...
	for (int j = 0; j < num_channels8; j++)
	{
		...
		deconv(img_fltr_p7 + j*rows*cols, all_tmp + (j * hr_pixels), weights_layer8 + j*filtersize8, cols, rows, scale);
	}
	
	// 3. FASE PENJUMLAHAN PARALEL (SPATIAL REDUCTION)
	// Kita ubah sumbu paralelnya, bukan per channel, tapi per PIKSEL.
	#pragma omp parallel for
	for (int p = 0; p < hr_pixels; p++)
	{
		...
		*(img_hr + p) = sum + biases_layer8;
	}
	t_end = omp_get_wtime();
	layer_times[7] += (t_end - t_start);
