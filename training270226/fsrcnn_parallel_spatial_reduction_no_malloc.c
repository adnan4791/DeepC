...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale, double *all_tmp);
...
int main(int argc, char *argv[])
{
	...
	// Pre-allocate all_tmp for Layer 8 deconvolution (Memory Pre-allocation)
	// Dialokasikan sekali di sini, bukan 150 kali di dalam FSRCNN()
	int num_channels8 = 56;
	int hr_pixels = outCols * outRows;
	...
	for (int fcnt = 0; fcnt < num; fcnt++)
	{
		...
		FSRCNN(outP_tmp, inP_tmp, inRows, inCols, scale, all_tmp);
...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale, double *all_tmp)
{
	...
	// all_tmp sudah di-pre-allocate di main(), tidak perlu malloc di sini
