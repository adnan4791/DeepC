...
void init_pool(int num_buffers, int buffer_size) {
    pool_size   = num_buffers;
    buffer_pool = (Buffer*)malloc(num_buffers * sizeof(Buffer));
    for (int i = 0; i < num_buffers; i++) {
        buffer_pool[i].data   = (double*)malloc(buffer_size * sizeof(double));
...
double* get_buffer(int size) {
    ...
    pthread_mutex_unlock(&pool_mutex);
    // fallback: alokasi dinamis jika pool habis
...
int main(int argc, char *argv[]) {
    ...
    // ========== Inisialisasi buffer pool ==========
    // Ukuran maks: layer1/7 output = 56 * inRows * inCols
    int max_buf = inRows * inCols * 56;
    init_pool(32, max_buf);
