...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale)
{
	...
	int hr_rows = rows * scale;
	int hr_cols = cols * scale;
	int hr_pixels = hr_rows * hr_cols; // Total piksel gambar HR

	// Parameter deconv (unrolled dari fungsi deconv)
	int border = 1;
	int fsize = 9;
	...
	// Pre-pad semua 56 channel input (unrolled dari deconv: pad_image)
	double **padded_inputs = (double **)malloc(num_channels8 * sizeof(double *));
	for (int j = 0; j < num_channels8; j++) {
		padded_inputs[j] = (double *)malloc(rows_pad * cols_pad * sizeof(double));
		pad_image(img_fltr_p7 + j * rows * cols, padded_inputs[j], rows, cols, border);
	}

	// Offset crop (unrolled dari deconv: bagian crop img_output_tmp -> img_output)
	int crop_r_offset = ((fsize + 1) / 2) + scale * border - 1;
	int crop_c_offset = ((fsize + 1) / 2) + scale * border - 1;
	int cols_out_pad_full = (cols_pad * scale) + fsize - 1;

	// FUSED LOOP: loop deconv di-unroll & di-fusion dengan loop reduction
	// Sekarang 1 loop paralel per HR pixel (bukan per channel)
	#pragma omp parallel for
	for (int p = 0; p < hr_pixels; p++)
	{
		...
		// Map ke koordinat di img_output_tmp (unrolled dari deconv crop)
		int i_tmp = r_out + crop_r_offset;
		int j_tmp = c_out + crop_c_offset;

		double sum = 0.0;

		// Loop over semua 56 channel (fused: dulu ini loop terpisah di reduction)
		for (int ch = 0; ch < num_channels8; ch++)
		{
			double *padded = padded_inputs[ch];
			double *kern = weights_layer8 + ch * filtersize8;

			double pixel_val = 0.0;

			// Gather: unrolled dari deconv scatter loop
			// Di deconv asli, input(i,j) scatter ke output(idx..idx+fsize-1, idy..idy+fsize-1)
			// dimana idx=i*scale, idy=j*scale
			// Sekarang di-invert: untuk output(i_tmp,j_tmp), cari input(i,j) yang berkontribusi
			// yaitu: i ∈ [ceil((i_tmp-fsize+1)/scale), floor(i_tmp/scale)]
			int i_start = (i_tmp - fsize + 1 + scale - 1) / scale; // ceil
			if (i_start < 0) i_start = 0;
			int i_end = i_tmp / scale;
			if (i_end >= rows_pad) i_end = rows_pad - 1;

			int j_start = (j_tmp - fsize + 1 + scale - 1) / scale; // ceil
			if (j_start < 0) j_start = 0;
			int j_end = j_tmp / scale;
			if (j_end >= cols_pad) j_end = cols_pad - 1;

			// Unrolled nested loop dari deconv (loop kernel × input)
			for (int i = i_start; i <= i_end; i++)
			{
				int kr = i_tmp - i * scale; // posisi kernel row
				for (int jj = j_start; jj <= j_end; jj++)
				{
					int kc = j_tmp - jj * scale; // posisi kernel col
					int cnt_img = i * cols_pad + jj;
					int cnt_kernel = kr * fsize + kc;
					pixel_val += padded[cnt_img] * kern[cnt_kernel];
				}
			}

			sum += pixel_val; // fused reduction: langsung akumulasi
		}
