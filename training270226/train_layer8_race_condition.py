...
def train():
    ...
    criterion = nn.MSELoss() # MSELoss (Direct Correlation to PSNR)
    optimizer = optim.Adam(model.parameters(), lr=0.00001)
    
    epochs = 40 # Anda bisa menaikkan ini nanti
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (layer7_input, hr_target) in enumerate(dataloader):
            ...
            #     print(f"Batas Bawah Target: {hr_target.min().item()}")
            #     print(f"Batas Atas Target: {hr_target.max().item()}")
            
            # Hitung error terhadap gambar HR yang sempurna
            # loss = criterion(output, hr_target)
            # loss_reconstruction = criterion(output, hr_target)
            # loss_ssim = 1 - ssim(output, hr_target, data_range=1.0, size_average=True)
            
            # 2. Loss L1: Membantu memperbaiki warna piksel yang meleset
            # loss_l1 = criterion(output, hr_target)
            
            # PENALTI L1: "Rem" agar PyTorch tidak menaikkan bobot gila-gilaan saat ada tabrakan thread
            # lambda_l1 = 0.00001
            # loss_penalty = lambda_l1 * torch.sum(torch.abs(model.deconv.weight))
            
            # loss = (0.8 * loss_ssim) + (0.2 * loss_l1) + loss_penalty
            loss = criterion(output, hr_target)
