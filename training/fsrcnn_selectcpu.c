...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale)
{
	...
	double bias_tmp;
	start_timer();
    #pragma omp parallel for firstprivate(kernel,cnt_weight,img_fltr_p1,bias_tmp) num_threads(nworkers)
	...
	cnt_weight = 0;
	start_timer();
    #pragma omp parallel for firstprivate(biases_layer2) num_threads(nworkers)
