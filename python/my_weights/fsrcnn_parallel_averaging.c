...
int main(int argc, char *argv[])
{
	...
	// Jumlah averaging runs (default=3, bisa diatur via argument ke-3)
	int N_AVG = 3;
	if (argc > 3) {
		N_AVG = atoi(argv[3]);
		if (N_AVG < 1) N_AVG = 1;
	...
	int hrSize = outCols * outRows;

	printf("=== FSRCNN Statistical Averaging ===\n");
	printf("  N_AVG = %d (jalankan FSRCNN %d kali per frame, rata-ratakan)\n", N_AVG, N_AVG);
	printf("  Threads = %d\n", omp_get_max_threads());
	printf("====================================\n");
	...
	// Buffer akumulator untuk averaging
	double *accum_buf = (double *)malloc(hrSize * sizeof(double));
	// Buffer sementara untuk setiap run FSRCNN
	double *run_buf = (double *)malloc(hrSize * sizeof(double));
	...
	for (int fcnt = 0; fcnt < num; fcnt++)
	{
		...
		// Reset akumulator ke 0
		memset(accum_buf, 0, hrSize * sizeof(double));

		// Jalankan FSRCNN N_AVG kali, akumulasi hasil
		for (int run = 0; run < N_AVG; run++)
		{
			FSRCNN(run_buf, inP_tmp, inRows, inCols, scale);
			// Tambahkan hasil run ke akumulator
			for (int p = 0; p < hrSize; p++)
			{
				accum_buf[p] += run_buf[p];
			}
		}

		// Rata-ratakan: bagi akumulator dengan N_AVG
		for (int p = 0; p < hrSize; p++)
		{
			outP_tmp[p] = (accum_buf[p] / (double)N_AVG) * 255.0;
		}
		// ========== END AVERAGING ==========
