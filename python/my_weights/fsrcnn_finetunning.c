...
// Modified by: [Your Name] to generate training data for fine-tuning
...
void FSRCNNCritical(double *img_hr, double *img_lr, int rows, int cols, int scale);
...
void save_yuv_frame(double *img, const char *filename, int cols, int rows);
...
{
    if (argc < 3) {
        printf("Usage: %s input.yuv output.yuv [train]\n", argv[0]);
        printf("  Jika argumen ketiga adalah 'train', maka akan menghasilkan dataset training di folder train_data/\n");
        return 1;
    }
    char *inFile = argv[1];
    char *outFile = argv[2];
    int generate_training = (argc >= 4 && strcmp(argv[3], "train") == 0);
    ...
    // Baca semua bobot (sama seperti sebelumnya)
    ...
    FILE *biases_layer8_ptr = fopen("biasess_layer8.txt", "r");
    if (biases_layer8_ptr == NULL) { printf("Error reading biasess_layer8.txt\n"); return 1; }
    ...
    if (!inBuf || !outBuf || !inBuf_tmp || !outBuf_tmp) {
        printf("Memory allocation failed\n");
        return 1;
    }

    // Jika generate training, buat folder-folder yang diperlukan
    if (generate_training) {
        system("mkdir -p train_data");
        system("mkdir -p train_data/gt");
        for (int r = 0; r < 30; r++) {
            char folder[100];
            sprintf(folder, "mkdir -p train_data/run%d", r);
            system(folder);
        }
        printf("Folder train_data/gt dan train_data/run0..29 telah dibuat.\n");
    }

    // Proses setiap frame
    for (int fcnt = 0; fcnt < num; fcnt++) {
        // Baca komponen Y
        size_t bytes_read = fread(inBuf, sizeof(unsigned char), inCols * inRows, inFp);
        if (bytes_read != (size_t)(inCols * inRows)) {
            printf("End of file after frame %d\n", fcnt);
            break;
        }

        // Konversi ke double [0,1]
        for (int i = 0; i < inRows; i++) {
            for (int j = 0; j < inCols; j++) {
                int cnt = i * inCols + j;
                inBuf_tmp[cnt] = inBuf[cnt] / 255.0;
            }
        }

        if (generate_training) {
            // --- Generate 30 run dengan race condition ---
            // Simpan input LR (opsional, untuk referensi)
            // char lr_filename[200];
            // sprintf(lr_filename, "train_data/lr/frame_%04d.yuv", fcnt);
            // save_yuv_frame(inBuf_tmp, lr_filename, inCols, inRows);

            for (int run = 0; run < 30; run++) {
                // Copy input (karena FSRCNN tidak mengubah input)
                double *input_copy = (double *)malloc(inCols * inRows * sizeof(double));
                memcpy(input_copy, inBuf_tmp, inCols * inRows * sizeof(double));

                // Panggil FSRCNN (dengan race condition)
                FSRCNN(outBuf_tmp, input_copy, inRows, inCols, scale);

                // Simpan output
                char out_filename[200];
                sprintf(out_filename, "train_data/run%d/frame_%04d.yuv", run, fcnt);
                save_yuv_frame(outBuf_tmp, out_filename, outCols, outRows);

                free(input_copy);
            }

            // --- Generate ground truth (versi deterministik) ---
            // Dengan menggunakan 1 thread, tidak ada race condition
            omp_set_num_threads(1);
            double *input_copy = (double *)malloc(inCols * inRows * sizeof(double));
            memcpy(input_copy, inBuf_tmp, inCols * inRows * sizeof(double));
            FSRCNNCritical(outBuf_tmp, input_copy, inRows, inCols, scale);
            char gt_filename[200];
            sprintf(gt_filename, "train_data/gt/frame_%04d.yuv", fcnt);
            save_yuv_frame(outBuf_tmp, gt_filename, outCols, outRows);
            free(input_copy);
            omp_set_num_threads(omp_get_max_threads()); // Kembalikan ke jumlah thread semula

            // Untuk U dan V, kita tidak perlu memproses karena training hanya pada Y
            // Tapi kita harus membaca U dan V dari file agar pointer file tetap maju
            fread(inBuf, sizeof(unsigned char), inCols * inRows / 4, inFp); // U
            fread(inBuf, sizeof(unsigned char), inCols * inRows / 4, inFp); // V
        } else {
            // Mode normal: proses seperti biasa (dengan race condition)
            FSRCNN(outBuf_tmp, inBuf_tmp, inRows, inCols, scale);

            // Konversi ke uint8 dan tulis ke output file
            for (int i = 0; i < outRows; i++) {
                for (int j = 0; j < outCols; j++) {
                    int cnt = i * outCols + j;
                    outBuf_tmp[cnt] = outBuf_tmp[cnt] * 255.0;
                }
            }
            double_2_uint8(outBuf_tmp, outBuf, outCols, outRows);
            fwrite(outBuf, sizeof(unsigned char), outCols * outRows, outFp);

            // Baca U dan V, lalu tulis dengan replikasi sederhana
            // U Component
    ...
    if (generate_training) {
        printf("Dataset training selesai dibuat di folder train_data/\n");
    }
    return 0;
}

// ==================== Fungsi tambahan untuk menyimpan frame YUV ====================
void save_yuv_frame(double *img, const char *filename, int cols, int rows) {
    unsigned char *buf = (unsigned char *)malloc(cols * rows * sizeof(unsigned char));
    if (!buf) {
        printf("Error: cannot allocate buffer for saving frame\n");
        return;
    }
    for (int i = 0; i < rows * cols; i++) {
        double val = img[i] * 255.0;
        if (val < 0) val = 0;
        if (val > 255) val = 255;
    ...
    if (fp) {
        fwrite(buf, sizeof(unsigned char), cols * rows, fp);
        fclose(fp);
    } else {
