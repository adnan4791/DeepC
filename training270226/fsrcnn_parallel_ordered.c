...
double *comp_w1, *comp_b1, *comp_w2, *comp_b2, *comp_w3, *comp_b3, *comp_w4, *comp_b4;
int comp_loaded = 0;
...
void compensator_forward_double(double *input, double *output, int H, int W);
void load_compensator_weights(const char *filename);
void conv2d_double(double *input, double *output, double *weight, double *bias,
                   int in_ch, int out_ch, int H, int W);
void relu_double(double *data, int n);
...
int main(int argc, char *argv[])
{
	...
	load_compensator_weights("compensator_weights.txt");
	...
	{
		// ========== EKSPERIMEN PEMBUKTIAN RACE CONDITION ==========
		// Lakukan hanya untuk frame pertama (fcnt == 0) agar tidak mengganggu seluruh video
		if (fcnt == 0) {
			#define N_RUNS 30
			double pixel_values[N_RUNS];
			
			// Tentukan posisi piksel tengah pada output
			int center_row = (inRows * scale) / 2;
			int center_col = (inCols * scale) / 2;
			int center_idx = center_row * (inCols * scale) + center_col;
			
			// Simpan input asli (Y component) karena akan diubah oleh FSRCNN
			double *input_copy = (double*)malloc(inCols * inRows * sizeof(double));
			memcpy(input_copy, inBuf_tmp, inCols * inRows * sizeof(double));
			
			// Lakukan N_RUNS kali pemanggilan FSRCNN
			for (int run = 0; run < N_RUNS; run++) {
				// Kembalikan input ke keadaan semula
				memcpy(inBuf_tmp, input_copy, inCols * inRows * sizeof(double));
				
				// Panggil FSRCNN (hasilnya di outBuf_tmp)
				FSRCNN(outBuf_tmp, inBuf_tmp, inRows, inCols, scale);
				
				// Catat nilai piksel pada posisi tengah
				pixel_values[run] = outBuf_tmp[center_idx];
			}
			
			// Hitung rata-rata dan standar deviasi
			double sum = 0.0, sum_sq = 0.0;
			for (int run = 0; run < N_RUNS; run++) {
				sum += pixel_values[run];
				sum_sq += pixel_values[run] * pixel_values[run];
			}
			double mean = sum / N_RUNS;
			double variance = (sum_sq - sum * mean) / (N_RUNS - 1); // sample variance
			double stddev = sqrt(variance);
			
			// Tampilkan hasil
			printf("===== EKSPERIMEN RACE CONDITION =====\n");
			printf("Piksel tengah (baris %d, kolom %d):\n", center_row, center_col);
			printf("Nilai dari %d kali run:\n", N_RUNS);
			for (int run = 0; run < N_RUNS; run++) {
				printf("  run %2d : %f\n", run, pixel_values[run]);
			}
			printf("Rata-rata   : %f\n", mean);
			printf("Std. deviasi: %g\n", stddev);
			if (stddev > 1e-6) {
				printf("KESIMPULAN: Standar deviasi > 1e-6 → HASIL NONDETERMINISTIK (race condition terbukti)\n");
			} else {
				printf("KESIMPULAN: Standar deviasi sangat kecil → mungkin deterministik atau race condition tidak berdampak pada piksel ini\n");
			}
			printf("=====================================\n");
			
			free(input_copy);
			
			// Lanjutkan dengan run berikutnya? Jika ingin melanjutkan proses frame pertama,
			// Anda mungkin perlu mengembalikan input lagi atau keluar dari loop.
			// Untuk eksperimen ini, kita bisa langsung keluar setelah frame pertama.
			// break; // jika ingin hanya proses frame pertama saja
		}
		// ========== AKHIR EKSPERIMEN ==========
...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale)
{
   ...
   for (int i=0;i<rows*scale;i++)
	...
	double *compensated = (double*)malloc(rows * scale * cols * scale * sizeof(double));
    if (!compensated) {
        printf("Error: cannot allocate compensated image\n");
        exit(1);
    }
    compensator_forward_double(img_hr, compensated, rows * scale, cols * scale);
    memcpy(img_hr, compensated, rows * scale * cols * scale * sizeof(double));
    free(compensated);
...
void load_compensator_weights(const char *filename) {
    ...
    if (!fp) {
        printf("Error: cannot open compensator weights file %s\n", filename);
        exit(1);
    }
    // Total jumlah float yang diketahui dari arsitektur:
    // conv1 weight: 64*1*9 = 576
    // conv1 bias: 64
    // conv2 weight: 64*64*9 = 36864
    // conv2 bias: 64
    // conv3 weight: 64*64*9 = 36864
    // conv3 bias: 64
    // conv4 weight: 1*64*9 = 576
    // conv4 bias: 1
    int total = 576 + 64 + 36864 + 64 + 36864 + 64 + 576 + 1;  // = 75073
    double *all = (double*)malloc(total * sizeof(double));
    if (!all) {
        printf("Error: cannot allocate memory for compensator weights\n");
        fclose(fp);
        exit(1);
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%f", &val) == 1) {
            if (idx >= total) {
                printf("Warning: too many floats in file, stopping\n");
                break;
            }
            all[idx++] = (double)val;
        }
    ...
    // Atur pointer ke masing-masing layer
    comp_w1 = all;
    comp_b1 = comp_w1 + 576;
    comp_w2 = comp_b1 + 64;
    comp_b2 = comp_w2 + 36864;
    comp_w3 = comp_b2 + 64;
    comp_b3 = comp_w3 + 36864;
    comp_w4 = comp_b3 + 64;
    comp_b4 = comp_w4 + 576;

    comp_loaded = 1;
    printf("Compensator weights loaded successfully.\n");
}

// Fungsi konvolusi 2D dengan padding=1, stride=1, kernel 3x3
// input  : array 1D berukuran in_ch * H * W (layout: channel, baris, kolom)
// output : array 1D berukuran out_ch * H * W
// weight : array 1D berukuran out_ch * in_ch * 9 (urutan: out_ch, in_ch, 3x3 row-major)
// bias   : array 1D berukuran out_ch
void conv2d_double(double *input, double *output, double *weight, double *bias,
                   int in_ch, int out_ch, int H, int W) {
    int pad = 1;
    int Hp = H + 2 * pad;
    ...
    // Alokasi input dengan padding (zero-padding)
    double *input_pad = (double*)calloc(in_ch * Hp * Wp, sizeof(double));
    if (!input_pad) {
        printf("Error: cannot allocate padded input in conv2d\n");
        exit(1);
    }

    // Salin bagian tengah
    for (int c = 0; c < in_ch; c++) {
        double *src = input + c * H * W;
        double *dst = input_pad + c * Hp * Wp;
        for (int i = 0; i < H; i++) {
            for (int j = 0; j < W; j++) {
                dst[(i + pad) * Wp + (j + pad)] = src[i * W + j];
            }
        }
    }
    ...
    for (int oc = 0; oc < out_ch; oc++) {
        double b = bias[oc];
        double *w = weight + oc * in_ch * 9;  // pointer ke kernel untuk output channel oc

        for (int i = 0; i < H; i++) {
            for (int j = 0; j < W; j++) {
                double sum = b;
                int base = (i + pad) * Wp + (j + pad);  // indeks pusat di padded input

                for (int ic = 0; ic < in_ch; ic++) {
                    double *w_ic = w + ic * 9;          // kernel untuk input channel ic
                    double *pad_ic = input_pad + ic * Hp * Wp;

                    // 3x3 window
                    sum += w_ic[0] * pad_ic[base - Wp - 1];
                    sum += w_ic[1] * pad_ic[base - Wp];
                    sum += w_ic[2] * pad_ic[base - Wp + 1];
                    sum += w_ic[3] * pad_ic[base - 1];
                    sum += w_ic[4] * pad_ic[base];
                    sum += w_ic[5] * pad_ic[base + 1];
                    sum += w_ic[6] * pad_ic[base + Wp - 1];
                    sum += w_ic[7] * pad_ic[base + Wp];
                    sum += w_ic[8] * pad_ic[base + Wp + 1];
                }

                output[oc * H * W + i * W + j] = sum;
            }
        }
    }
...
// Forward pass compensator
// input  : gambar 1 channel ukuran H x W (array double, layout baris-mayor)
// output : gambar 1 channel hasil kompensasi (alokasi terpisah)
void compensator_forward_double(double *input, double *output, int H, int W) {
    if (!comp_loaded) {
        printf("Error: compensator weights not loaded.\n");
        exit(1);
    }
    ...
    double *f1 = (double*)malloc(f1_size * sizeof(double));
    double *f2 = (double*)malloc(f2_size * sizeof(double));
    double *f3 = (double*)malloc(f3_size * sizeof(double));

    if (!f1 || !f2 || !f3) {
        printf("Error: cannot allocate feature maps in compensator\n");
        exit(1);
    }

    // Layer 1: conv + ReLU
    conv2d_double(input, f1, comp_w1, comp_b1, 1, 64, H, W);
    relu_double(f1, f1_size);

    // Layer 2: conv + ReLU
    conv2d_double(f1, f2, comp_w2, comp_b2, 64, 64, H, W);
    relu_double(f2, f2_size);

    // Layer 3: conv + ReLU
    conv2d_double(f2, f3, comp_w3, comp_b3, 64, 64, H, W);
    relu_double(f3, f3_size);

    // Layer 4: conv (tanpa ReLU)
    conv2d_double(f3, output, comp_w4, comp_b4, 64, 1, H, W);
