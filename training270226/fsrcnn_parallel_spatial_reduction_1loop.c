...
void FSRCNN(double *img_hr, double *img_lr, int rows, int cols, int scale)
{
	...
	int hr_pixels = rows_out * cols_out;
	int border8 = 1;
	int rows_pad8 = rows + 2 * border8;
	int cols_pad8 = cols + 2 * border8;

	img_fltr_p7 = img_fltr_7;

	// Pad semua input channel terlebih dahulu
	double *all_padded = (double *)malloc(num_channels8 * rows_pad8 * cols_pad8 * sizeof(double));
	for (int ch = 0; ch < num_channels8; ch++)
	{
		pad_image(img_fltr_p7 + ch*rows*cols, all_padded + ch*rows_pad8*cols_pad8, rows, cols, border8);
	}

	// Konstanta untuk offset crop (dihitung sekali di luar loop)
	int crop_offset_r = ((fsize8 + 1) / 2) + scale * border8 - 1;
	int crop_offset_c = crop_offset_r; // sama karena simetris
	int padded_size = rows_pad8 * cols_pad8;

	// SATU LOOP GABUNGAN: Paralel per PIKSEL HR
	// Deconv di-inline menggunakan pendekatan GATHER (bukan scatter).
	// Step by 'scale' untuk menghindari branch dan modulo di hot loop.
	...
	for (int p = 0; p < hr_pixels; p++)
	{
		...
		int i_tmp = oy + crop_offset_r;
		int j_tmp = ox + crop_offset_c;

		// Hitung starting k_r dan k_c agar (i_tmp - k_r) % scale == 0
		// Lalu step by 'scale' sehingga semua iterasi valid, tanpa branch
		int k_r_start = i_tmp % scale;
		int k_c_start = j_tmp % scale;

		double sum = biases_layer8;

		for (int ch = 0; ch < num_channels8; ch++)
		{
			double *padded = all_padded + ch * padded_size;
			double *kern = weights_layer8 + ch * filtersize8;

			double ch_val = 0.0;
			for (int k_r = k_r_start; k_r < fsize8; k_r += scale)
			{
				int ii = (i_tmp - k_r) / scale;
				if (ii < 0 || ii >= rows_pad8) continue;

				for (int k_c = k_c_start; k_c < fsize8; k_c += scale)
				{
					int jj = (j_tmp - k_c) / scale;
					if (jj < 0 || jj >= cols_pad8) continue;

					ch_val += padded[ii * cols_pad8 + jj] * kern[k_r * fsize8 + k_c];
				}
			}
			sum += ch_val;
		}
