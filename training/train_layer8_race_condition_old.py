...
def main():
    ...
    for epoch in range(CONFIG['num_epochs']):
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            weights = model.get_weights_flat()
            # Option 0: save as one line format decimal
            # np.savetxt(CONFIG['output_weights_file'], weights, fmt='%.10f')
            # Option 1: One value per line (compatible, reliable)
            # np.savetxt(CONFIG['output_weights_file'], weights, fmt='%.16e')
            # Option 2: All on one line like original model (uncomment if preferred)
            with open(CONFIG['output_weights_file'], 'w') as f:
